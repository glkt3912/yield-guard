...
用途地域・都市計画区域に加え、以下の法的制約が融資可否・出口戦略に直結する。詳細は [`domain-glossary.md` セクション9「物件属性・法的制約」](./domain-glossary.md#9-物件属性法的制約) を参照。

| 概念 | 関連 |
|------|------|
| 再建築不可 | 接道義務違反により建て替え不可。担保評価が大幅低下 |
| 接道義務（建築基準法第43条） | 幅員4m以上の道路に2m以上接していない場合に発生 |
| 42条2項道路（みなし道路） | 市街化区域内の旧来の狭路。セットバック後に再建築可 |
| 既存不適格 | 建ぺい率・耐震基準等の法改正により現行法不適合となった建物 |
