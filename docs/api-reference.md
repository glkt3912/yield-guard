...
物件の緯度経度からタイル座標を算出し、11 種類の MLIT API を並列呼び出しして投資適地スコア（0〜100）を返す。
...
| 駅乗降客数 | 0〜+20 | 5万人/日以上→+20 を上限に線形スケール |
...
- 11 API を goroutine で並列実行。個別 API の失敗はログのみ（部分スコアで返す）
...
- 各タイルで `calcScoreForTile`（11 API 並列）を実行
- タイル並列数はセマフォ（バッファ=5）で制御 → 最大同時 MLIT リクエスト数: 5 × 11 = 55
- 個別タイルの失敗はスキップしてログのみ（部分結果を返す）
- コンテキストキャンセル時は goroutine が即座に終了
- パニック発生時も `recover` でキャッチして results チャネルに送信（デッドロック防止）
- 24時間 TTL キャッシュが効くため、同一範囲の2回目以降はほぼゼロコスト
