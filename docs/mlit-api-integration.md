...
## 利用可能なAPIエンドポイント一覧（主要）

本プロジェクトで現在使用しているのは **XIT001** のみ。将来的な拡張候補を含め主要APIを記載する。

| API ID | 名称 | 用途 | 本PJ利用 |
|--------|------|------|---------|
| **XIT001** | 不動産価格（取引価格・成約価格）情報取得 | 土地取引価格の取得 | ✅ 使用中 |
| **XIT002** | 都道府県内市区町村一覧取得 | 市区町村コード一覧の動的取得 | 候補 |
| XCT001 | 鑑定評価書情報 | 地価公示・地価調査データ | 候補 |
| XPT001/XPT002 | ポイントAPI（価格情報） | 地図上の価格情報表示 | 候補 |
| XKT001〜XKT031 | 都市計画・施設・防災情報 | 用途地域・ハザード情報等 | 候補 |

> XIT002 を利用すると `types.go` の `Prefectures` マップのように静的に持っている市区町村一覧を動的取得に変更できる（issue #5 キャッシュ実装と合わせて検討）。
