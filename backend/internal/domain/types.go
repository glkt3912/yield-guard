...
type YieldScenario struct {
	...
	GrossYield float64 `json:"grossYield"` // 表面利回り（満室想定年収/物件価格）
}
...
type InvestmentResult struct {
	...
	// GrossYield は表面利回り（満室想定年収/物件価格[土地+建物]）。市場慣行（物件広告・REINS）に合わせ諸費用を分母に含めない。
	GrossYield float64 `json:"grossYield"`
	// GrossYieldOnTotalInvestment は総投資利回り（満室想定年収/総投資額[土地+建物+諸経費]）。実際の投下資本に対する利回り。
	GrossYieldOnTotalInvestment float64 `json:"grossYieldOnTotalInvestment"`
	NetYield                    float64 `json:"netYield"` // 実質利回り（実効収入-経費)/総投資額）。GrossYield と分母が異なる点に注意。
	IsAboveYieldTarget          bool    `json:"isAboveYieldTarget"`
	YieldTarget                 float64 `json:"yieldTarget"` // 目標表面利回り（例: 0.08）。物件価格ベースの GrossYield で判定。
