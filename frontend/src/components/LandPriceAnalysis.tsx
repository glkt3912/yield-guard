...
export function LandPriceAnalysis({
  comparison,
  input,
  theoreticalPrice,
  stationRidership,
  ...
  const allUrbanRisks: UrbanRisk[] = [
    ...
    ...(externalUrbanRisks ?? []).filter(
      (r) => !(stats.urbanRisks ?? []).some((e) => e.code === r.code)
    ),
  ];

  const landValueSection = (() => {
    if (!input || stats.count === 0 || stats.medianTsubo === 0 || comparison.inputArea === 0)
      return null;
          ...
          <div className="flex items-start gap-3 rounded-md border border-amber-200 bg-amber-50 p-4 text-amber-800">
            ...
            <div>
              ...
              <p className="text-xs mt-1">
                国交省データベースに該当期間の宅地取引実績がありません。都道府県・市区町村・取得期間を変更してお試しください。
              </p>
            </div>
          </div>
        </CardContent>
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        {populationForecast &&
          ...
          (() => {
            const style =
              POPULATION_TREND_STYLE[populationForecast.trend] ??
              POPULATION_TREND_STYLE["現状維持"];
            const displayYears = [2020, 2030, 2040, 2050];
            const snapshotMap = Object.fromEntries(
              populationForecast.snapshots.map((s) => [s.year, s.pop])
            );
            ...
            return (
              <div className={`rounded-md border ${style.border} ${style.bg} p-3`}>
                    ...
                    return (
                      <div key={year} className="text-center">
                        ...
                        <p className={`text-xs font-semibold ${style.text}`}>
                          {pop > 0 ? pop.toFixed(0) : "−"}人
                        </p>
                        {year !== 2020 && base2020 > 0 && (
                          <p
                            className={`text-xs ${ratePct >= 0 ? "text-green-700" : "text-red-700"}`}
                          >
                            {ratePct >= 0 ? "+" : ""}
                            {ratePct.toFixed(0)}%
                          </p>
                ...
                <p className={`mt-2 text-xs ${style.text} opacity-70`}>
                  30年後変化率: {(populationForecast.changeRate30yr * 100).toFixed(0)}% ／
                  推定空室率増加: +{(populationForecast.vacancyRateDelta * 100).toFixed(0)}%pt
                </p>
              </div>
            );
          })()}
          ...
          (() => {
            ...
            return (
              <div className="rounded-md border bg-muted/30 p-3 space-y-2">
                ...
                <p className="text-sm">
                  推定利回り範囲: {(benchmark.estimatedYieldMin * 100).toFixed(1)}% 〜{" "}
                  {(benchmark.estimatedYieldMax * 100).toFixed(1)}%（中央値:{" "}
                  {(benchmark.estimatedYieldTypical * 100).toFixed(1)}%）
                </p>
                <p className="text-xs text-muted-foreground">
                  ※ 土地面積 {input.landArea}m²・建物代 {Math.round(input.buildingCost / 10_000)}
                  万円 で試算
                </p>
                {input.landPrice + input.buildingCost > 0 && (
        ...
        {inputPricePerTsubo > 0 && (
          ...
          >
            ...
            <p className="text-xs mt-1">
              ...
              <span
                className={
                  diffFromMedian > 0 ? "text-red-600 font-bold" : "text-green-600 font-bold"
                }
              >
                {diffFromMedian > 0 ? "+" : ""}
                {formatTsubo(Math.abs(diffFromMedian))}（{diffFromMedian > 0 ? "割高" : "割安"}）
              </span>
            </p>
            {stats.lowDataWarning && (
          ...
          <div className="rounded-md border border-purple-200 bg-purple-50 p-3">
            ...
            <div className="space-y-1">
              {stationRidership.slice(0, 5).map((s) => {
                ...
                return (
                  <div
                    key={`${s.stationName}-${s.lineName}`}
                    className="flex items-center justify-between text-xs"
                  >
                    <span className="text-purple-900">
                      {s.stationName}（{s.lineName}）
                    </span>
                    <div className="flex items-center gap-2">
        ...
        {landAppraisal &&
          landAppraisal.appraisalCount > 0 &&
          (() => {
            const transactionMedianPerSqm =
              stats.medianTsubo > 0 ? stats.medianTsubo / SQM_PER_TSUBO : 0;
            const ratio =
              transactionMedianPerSqm > 0 && landAppraisal.appraisalMedianPerSqm > 0
                ...
                <dl className="space-y-1 text-sm">
                  <div className="flex justify-between">
                    ...
                    <dd className="font-medium">
                      {Math.round(landAppraisal.appraisalMedianPerSqm).toLocaleString()}円/m²
                    </dd>
                  </div>
                  {transactionMedianPerSqm > 0 && (
                    <div className="flex justify-between">
                      ...
                      <dd className="font-medium">
                        {Math.round(transactionMedianPerSqm).toLocaleString()}円/m²
                      </dd>
                    </div>
                  )}
                  {ratio !== null && (
                    <div className="flex justify-between border-t pt-1">
                      ...
                      <dd
                        className={`font-bold ${ratio > 1.1 ? "text-red-700" : ratio < 0.9 ? "text-green-700" : ""}`}
                      >
                        {ratio.toFixed(2)}倍
                        {ratio > 1.1
                          ? "（取引が公示を上回る）"
                  ...
                  <div className="flex justify-between">
                    ...
                    <dd className={`font-bold ${style.text}`}>
                      {landAppraisal.trendLabel}（{landAppraisal.appraisalTrend >= 0 ? "+" : ""}
                      {(landAppraisal.appraisalTrend * 100).toFixed(1)}%）
                    </dd>
                  </div>
              ...
              <div className={`rounded-md border p-3 ${color}`}>
                ...
                <dl className="space-y-1 text-sm">
                  ...
                  <div className="flex justify-between border-t pt-1">
                    ...
                    <dd
                      className={`font-bold ${isOverpriced ? "text-red-700" : isUnderpriced ? "text-green-700" : ""}`}
                    >
                      {deviationPct > 0 ? "+" : ""}
                  ...
                  {hasStationData && (
                    <div className="flex justify-between text-xs text-muted-foreground">
                      ...
                      <dd>
                        {stationCorrection >= 0 ? "+" : ""}
                        {(stationCorrection * 100).toFixed(1)}%
                      </dd>
                    </div>
                  )}
                  {hasRidershipData && ridershipScore && (
                    <div className="flex justify-between text-xs text-muted-foreground">
                      <dt>
                        需要スコア補正（スコア:{" "}
                        {RIDERSHIP_SCORE_LABEL[ridershipScore]?.label ?? ridershipScore}）
                      </dt>
                      <dd>
                        {ridershipCorrection >= 0 ? "+" : ""}
                        {(ridershipCorrection * 100).toFixed(1)}%
                      </dd>
                    </div>
                  )}
                </dl>
