...
describe("LoanOptimizationPanel", () => {
  it("DSCR >= 1.0 のとき緑バッジ（安全）を表示する", () => {
    ...
    render(
      <LoanOptimizationPanel result={result} loanMethod="equal-payment" onLoanMethodChange={vi.fn()} />
    );
  ...
  it("返済方式セレクタが初期値を表示し、変更時に onLoanMethodChange を呼ぶ", () => {
    ...
    const result = makeResult({ dscr: 1.2, ltvSensitivity: [] });
    render(
      <LoanOptimizationPanel result={result} loanMethod="equal-payment" onLoanMethodChange={onChange} />
    );
